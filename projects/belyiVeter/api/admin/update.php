<?php
    ...
    strlen($_POST["cost"]) > 0 )
    {
        ...
        if(isset($_FILES["image"]) && isset($_FILES["image"]["name"]) && strlen($_FILES["image"]["name"]) > 0) {
            ...
            $query = mysqli_query($con, "SELECT img FROM smartphones WHERE id=$id");
            ...
            $prep = mysqli_prepare($con, "UPDATE smartphones set  name=?, make=?,img=?,cost=? where id=$id");
            mysqli_stmt_bind_param($prep,"sssi", $name, $make,$path,$cost);
            mysqli_stmt_execute($prep);
        }
        else{
            $prep = mysqli_prepare($con,"UPDATE smartphones set  name=?, make=?,cost=? where id=$id");
